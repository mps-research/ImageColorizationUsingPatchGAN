...
class Trainable(tune.Trainable):
    ...
    def step(self):
        ...
        for gray_images, real_images in self.train_dataloader:
            ...
            outputs = self.netD(real_images)
            labels = torch.full(outputs.size(), 1., dtype=torch.float, device=self.device)
            errD_real = self.gan_criterion(outputs, labels)
            errD_real.backward()

            fake_images = self.netG(gray_images)
            labels.fill_(0.)
            outputs = self.netD(fake_images.detach())
...
if __name__ == '__main__':
    ...
    tune.run(
        ...
        config=config,
        resources_per_trial={'gpu': .5, 'cpu': 1},
        num_samples=1
    )
